    ...
    */
    ...
    public R save(@RequestBody YishengEntity yisheng, HttpServletRequest request){
        ...
        if(yishengEntity==null){
            ...
            return R.error(511,"账户或者医护手机号或者医护身份证号已经被使用");
        }
    }
...
//                            yishengEntity.setYishengName(data.get(0));                    //医护姓名 要改的
//                            yishengEntity.setYishengPhone(data.get(0));                    //医护手机号 要改的
//                            yishengEntity.setYishengIdNumber(data.get(0));                    //医护身份证号 要改的
//                            yishengEntity.setYishengPhoto("");//详情和图片
//                            yishengEntity.setZuozhenShijian(data.get(0));                    //坐诊时间 要改的
...
//                            yishengEntity.setYishengEmail(data.get(0));                    //医护邮箱 要改的
...
//                            yishengEntity.setCreateTime(date);//时间
                        ...
                        if(yishengEntities_yishengPhone.size() >0 ){
                            ...
                            return R.error(511,"数据库的该表中的 [医护手机号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
                         //医护身份证号
                        ...
                        if(yishengEntities_yishengIdNumber.size() >0 ){
                            ...
                            return R.error(511,"数据库的该表中的 [医护身份证号] 字段已经存在 存在数据为:"+repeatFields.toString());
                        }
    ...
    */
    ...
    public R login(String username, String password, String captcha, HttpServletRequest request) {
        ...
        String token = tokenService.generateToken(yisheng.getId(),username, "yisheng", "医护");
        R r = R.ok();
        r.put("token", token);
        r.put("role","医护");
...
//    	ValidatorUtils.validateEntity(user);
        ...
        YishengEntity yishengEntity = yishengService.selectOne(queryWrapper);
        if(yishengEntity != null)
            return R.error("账户或者医护手机号或者医护身份证号已经被使用");
        yisheng.setCreateTime(new Date());
        yishengService.insert(yisheng);
