            ...
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '药品名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.yaopinName" placeholder="药品名称" clearable></el-input>
                     </el-form-item>
                                 
                     <el-form-item :label="contents.inputTitle == 1 ? '药品类型' : ''">
                        <el-select v-model="searchForm.yaopinTypes" placeholder="请选择药品类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in yaopinTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('yaopin','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/yiyuandanganguanlixitong/upload/yaopinMuBan.xls"
                        >批量导入药品数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('yaopin','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="yaopinUploadSuccess"
                                :on-error="yaopinUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('yaopin','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "yaopin.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
                ...
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            {{scope.row.yaopinName}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="yaopinUuidNumber"
                                   header-align="center"
                                   label="药品编号">
                        <template slot-scope="scope">
                            {{scope.row.yaopinUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="yaopinPhoto"
                                     header-align="center"
                                     width="200"
                                     label="药品照片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.yaopinPhoto">
                                <img :src="$base.url+scope.row.yaopinPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="shengchanchangjia"
                                   header-align="center"
                                   label="生产厂家">
                        <template slot-scope="scope">
                            {{scope.row.shengchanchangjia}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="yaopinTypes"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('yaopin','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('yaopin','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('yaopin','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //级联表下拉框搜索条件
    //当前表下拉框搜索条件
              yaopinTypesSelectSearch : [],
            form:{
                id : null,
                yaopinName : null,
                yaopinUuidNumber : null,
                yaopinPhoto : null,
                shengchanchangjia : null,
                yaopinTypes : null,
                yaopinGongxiaoContent : null,
                yaopinJinjiContent : null,
                yaopinZhuyiContent : null,
                yaopinContent : null,
                insertTime : null,
                createTime : null,
            },
            ...
            json_fields: {
                ...
                //本表字段
                     '药品名称': "yaopinName",
                     '药品编号': "yaopinUuidNumber",
                     '药品照片': "yaopinPhoto",
                     '生产厂家': "shengchanchangjia",
                     '药品类型': "yaopinValue",
                     '录入时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'yaopin',//当前表表名,
                        sumColum : 'yaopin_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'yaopin_name',//分组字符串字段
                        // types : 'yaopin_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.yaopinName!= '' && this.searchForm.yaopinName!= undefined) {
                    params['yaopinName'] = '%' + this.searchForm.yaopinName + '%'
                }
                                 
                if (this.searchForm.yaopinTypes!= '' && this.searchForm.yaopinTypes!= undefined) {
                    params['yaopinTypes'] = this.searchForm.yaopinTypes
                }
                                                
                params['yaopinDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=yaopin_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.yaopinTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            yaopinUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "yaopin/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
