        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '档案编号' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.bingliUuidNumber" placeholder="档案编号" clearable></el-input>
                     </el-form-item>
                                                                                                 
                     <el-form-item :label="contents.inputTitle == 1 ? '身体状况' : ''">
                        <el-select v-model="searchForm.bingliTypes" placeholder="请选择身体状况">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in bingliTypesSelectSearch"
                               v-bind:key="index"
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '医护姓名' : ''">
                        <el-input prefix-icon="el-icon-search" v-model="searchForm.yishengName" placeholder="医护姓名" clearable></el-input>
                    </el-form-item>
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('bingli','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/yiyuandanganguanlixitong/upload/bingliMuBan.xls"
                        >批量导入档案数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('bingli','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="bingliUploadSuccess"
                                :on-error="bingliUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('bingli','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "bingli.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="yishengPhoto"
                        ...
                        <template slot-scope="scope">
                            <div v-if="scope.row.yishengPhoto">
                                <img :src="$base.url+scope.row.yishengPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="bingliUuidNumber"
                                   header-align="center"
                                   label="档案编号">
                        <template slot-scope="scope">
                            {{scope.row.bingliUuidNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   ...
                                   label="花费金额">
                        <template slot-scope="scope">
                            {{scope.row.huafeijine}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="xiacijiuzhenTime"
                                   header-align="center"
                                   label="下次就诊时间">
                        <template slot-scope="scope">
                            {{scope.row.xiacijiuzhenTime}}
                        </template>
                    </el-table-column>
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="bingliNianling"
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('bingli','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('bingli','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('bingli','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              bingliTypesSelectSearch : [],
            form:{
                id : null,
                bingliUuidNumber : null,
                yonghuId : null,
                yishengId : null,
                jianchaTime : null,
                bingrenzishuContent : null,
                jianchajieguoContent : null,
                yishengkaifangContent : null,
                yishengjianyiContent : null,
                bingliYibao : null,
                huafeijine : null,
                xiacijiuzhenTime : null,
                bingliNianling : null,
                bingliTypes : null,
                insertTime : null,
                createTime : null,
            ...
            json_fields: {
                //级联表字段
                     ...
                     '邮箱': 'yonghuEmail',
                //本表字段
                     '档案编号': "bingliUuidNumber",
                     '检查时间': "jianchaTime",
                     '医保信息': "bingliYibao",
                     '花费金额': "huafeijine",
                     '下次就诊时间': "xiacijiuzhenTime",
                     '年龄': "bingliNianling",
                     '身体状况': "bingliValue",
                     '录入时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'bingli',//当前表表名,
                        sumColum : 'bingli_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'bingli_name',//分组字符串字段
                        // types : 'bingli_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.keshiTypes!= '' && this.searchForm.keshiTypes!= undefined) {
                    params['keshiTypes'] = this.searchForm.keshiTypes
                }
                ...
                if (this.searchForm.bingliUuidNumber!= '' && this.searchForm.bingliUuidNumber!= undefined) {
                    params['bingliUuidNumber'] = '%' + this.searchForm.bingliUuidNumber + '%'
                }
                                                                                                 
                if (this.searchForm.bingliTypes!= '' && this.searchForm.bingliTypes!= undefined) {
                    params['bingliTypes'] = this.searchForm.bingliTypes
                }
                
                params['bingliDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //查询级联表搜索条件所有列表
                this.$http({
                    url: "dictionary/page?dicCode=keshi_types&page=1&limit=100",
                    method: "get",
                    page: 1,
                    limit: 100,
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.keshiTypesSelectSearch = data.data.list;
                    }
                });
                this.$http({
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=bingli_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.bingliTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            bingliUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "bingli/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
