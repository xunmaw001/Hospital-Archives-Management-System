        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '科室名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.keshiName" placeholder="科室名称" clearable></el-input>
                     </el-form-item>
                                 
                     <el-form-item :label="contents.inputTitle == 1 ? '科室类型' : ''">
                        <el-select v-model="searchForm.keshiTypes" placeholder="请选择科室类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in keshiTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('keshi','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/yiyuandanganguanlixitong/upload/keshiMuBan.xls"
                        >批量导入科室数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('keshi','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="keshiUploadSuccess"
                                :on-error="keshiUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('keshi','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "keshi.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="keshiUuidNumber"
                                   header-align="center"
                                   label="科室编号">
                        <template slot-scope="scope">
                            {{scope.row.keshiUuidNumber}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="keshiPhoto"
                                     header-align="center"
                                     width="200"
                                     label="科室照片">
                        <template slot-scope="scope">
                            <div v-if="scope.row.keshiPhoto">
                                <img :src="$base.url+scope.row.keshiPhoto" width="100" height="100">
                            </div>
                            <div v-else>无图片</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('keshi','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('keshi','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('keshi','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
            ...
            form:{
                id : null,
                keshiName : null,
                keshiUuidNumber : null,
                keshiPhoto : null,
                shengchanchangjia : null,
                keshiTypes : null,
                keshiGongxiaoContent : null,
                keshiJinjiContent : null,
                keshiZhuyiContent : null,
                keshiContent : null,
                insertTime : null,
                createTime : null,
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'keshi',//当前表表名,
                        sumColum : 'keshi_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'keshi_name',//分组字符串字段
                        // types : 'keshi_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.keshiName!= '' && this.searchForm.keshiName!= undefined) {
                    params['keshiName'] = '%' + this.searchForm.keshiName + '%'
                }
                ...
                params['keshiDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
            ...
            // 导入功能上传文件成功后调用导入方法
            keshiUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "keshi/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
